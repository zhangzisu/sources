...
struct tag_t {
    ...
    inline int valueAt(int l, int r) {
...
inline void change(int n, int l, int r, tag_t v) {
    ...
    int mid = (l + r) >> 1;
    if (v.a > tag[n].a) {
        if (v.valueAt(l, r) > tag[n].valueAt(l, r)) {
            change(n << 1, l, mid, tag[n]);
            tag[n] = v;
        } else {
...
int main() {
    for (int op, l, r, a, b; m; m--) {
        ...
        if (op == 1) {
            ...
            a = io.getint();
            io.put(trim(hls::query(1, 1, n, a) + jzm::query(1, 1, n, a, a)), '\n');
        }
    }
    return 0;
}