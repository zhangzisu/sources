...
} io(fopen("bracket.in", "r"), fopen("bracket.out", "w"));
#define MAXN 2000010
int head[MAXN], to[MAXN], next[MAXN], tot = 0, root, ans = 0;
inline void $(int u, int v) { next[tot] = head[u], to[tot] = v, head[u] = tot++; }
int n = io.getint(), p[MAXN], v[MAXN], Q[MAXN], H = 1, T = 0, amin[MAXN], amax[MAXN], bmin[MAXN], bmax[MAXN], cmin[MAXN], cmax[MAXN], L[MAXN], R[MAXN];
int main() {
