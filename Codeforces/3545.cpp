...
#define MAXN 200010
#define MAXM 500010
int n, m, q, h[MAXN], g[MAXN], p[MAXN], dat[MAXM], N;
inline int GetCGC(int x) { return std::lower_bound(dat + 1, dat + N + 1, x) - dat; }
...
int root[MAXN], L[MAXN], R[MAXN], sum[MAXN], cnt = 0;
...
int query(int n0, int n1, int l, int r, int k) {
    if (l == r) return l;
    int now = sum[L[n0]] - sum[L[n1]], mid = (l + r) >> 1;
    if (now >= k) return query(L[n0], L[n1], 1, mid, k);
    return query(R[n0], R[n1], mid + 1, r, k - now);
}
void dfs(int x) {
    fsa[x] = ++dfsclk;
    root[dfsclk] = root[dfsclk - 1];
    if (x <= n) insert(root[dfsclk], root[dfsclk - 1], 1, N, g[x]);
...
int main() {
    ...
    for (int i = 1; i <= m; i++) scanf("%d%d%d", &e[i].u, &e[i].v, &e[i].w), dat[++N] = e[i].w;
    std::sort(e + 1, e + m + 1);
    dat[++N] = 1;
    ...
    for (int i = 1; i <= n; i++) printf("%5d", g[i]);
    puts("");
    for (int i = 1; i <= n; i++)
