...
int main() {
	...
	scanf("%d%d", &a, &b);
	SQRT(f, g, n);
	for (int i = 0; i < n; i++) h[i] = 1LL * INV2 * (h[i] + g[i]) % MOD;
	LN(h, l, n);
	for (int i = 0; i < n; i++) l[i] = 1LL * (a + 1) * l[i] % MOD;
	EXP(l, h, n);
	INV(g, l, n);
	DFT(h, n << 1);
	DFT(l, n << 1);
	for (int i = 0; i < (n << 1); i++) h[i] = 1LL * h[i] * l[i] % MOD;
	IDFT(h, n << 1);
	for (int i = 1; i <= b; i++) printf("%d ", i <= a ? h[i] : 0);
	puts("");
	return 0;
}