...
inline double calc_2d(double y) {
	...
	for (int i = cone_2d_l; i <= cone_2d_r; i++) {
		...
		double z = cone.h - sqrt(sqr(y - cone.y) + cone.c);
		ret = std::max(ret, z);
	}
	return ret;
}
...
inline double calc_3d(double x) {
	...
	for (int i = cone_3d_l; i <= cone_3d_r; i++) {
		...
		double r = sqrt(sqr(cone.h) - c);
		cone_2d[++cone_2d_c] = {cone.y, cone.y - r, cone.y + r, c, cone.h};
	}
...
int main() {
	for (scanf("%d", &T); T; T--) {
		...
		for (int i = 1; i <= n; i++) {
			...
			cone_3d[i] = {x[i], x[i] - h[i], x[i] + h[i], y[i], h[i]};
		}
