...
int n = $(), m = $(), f[MAXN], root;
struct node{
	int l, r, max[4], min[4], val[4], dat;unsigned long long sum;
	inline node(int a = 0, int b = 0, int c = 0, int d = 0, int e = 0){
		...
		max[2] = min[2] = val[2] = c;
		max[3] = min[3] = val[3] = d;
		sum = (unsigned long long)(b - a) * (d - c) * e;
	}
}p[MAXN << 1];
...
inline bool ckr(const int &n){
	if(p[n].min[0] >= u && p[n].max[1] <= d && p[n].min[2] >= l && p[n].max[3] <= r)return 1;
	return 0;
}
inline unsigned long long calc(const int &n){
