...
inline ll cal() {
    ...
    int cp = 0;
    for (int i = 1; i <= ce; i++) {
        pool[++cp] = W(e[i].a - e[i].c, e[i].a << 1, 0, 0, e[i].b);
        pool[++cp] = W(e[i].b + e[i].c, e[i].a << 1 | 1, 1, e[i].b, 0);
    }
    ...
    for (int i = 1; i <= cp; i++) {
        ...
        if (pool[i].op == 1)
            ...
            ret += ask(pool[i].r) - ask(pool[i].l - 1);
    }
    cp = 0;
    for (int i = 1; i <= ce; i++) {
        pool[++cp] = W(e[i].a - e[i].b - e[i].c, e[i].a << 1, 0, e[i].b + 1, n);
        pool[++cp] = W(e[i].c, e[i].a << 1 | 1, 1, e[i].b, 0);
    }
    sort(pool + 1, pool + cp + 1, cmpw);
    for (int i = 1; i <= cp; i++) {
        if (i == 1 || pool[i].v != pool[i - 1].v) POS++;
...
void dfs(int x, int y, int dis, int ma, int mi) {
    ...
    e[++ce] = E(ma, mi, dis);
    for (int i = g[x]; i; i = nxt[i])
        if (ok[i] && v[i] != y) dfs(v[i], x, dis + 1, ma, mi);
}
void dfs2(int x, int y, int dis, int ma, int mi) {
    ...
    e[++ce] = E(ma, mi, dis);
    for (int i = g[x]; i; i = nxt[i])
        if (ok[i] && v[i] != y) dfs2(v[i], x, dis + 1, ma, mi);
}
void solve(int x) {
    int i;
    for (i = g[x]; i; i = nxt[i])
        if (ok[i]) {
            ...
            dfs(v[i], x, 1, x, x);
            long long tmp = cal();
            printf("DEBUG solve %d sub %d: %lld\n", x, v[i], tmp);
            ans -= tmp;
        }
    ce = 0;
    for (i = g[x]; i; i = nxt[i])
        if (ok[i]) dfs2(v[i], x, 1, x, x);
    long long tmp = cal();
    printf("DEBUG solve %d: %lld\n", x, tmp);
    ans += tmp;
    for (i = g[x]; i; i = nxt[i])
        if (ok[i]) {
            ok[i ^ 1] = 0;
