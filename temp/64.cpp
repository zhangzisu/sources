...
inline lnt count(lnt val) {
	lnt cnt = 0;
	for (int i = 1; i <= n; i++) {
		cnt += (std::upper_bound(b + 1, b + n + 1, val - a[i]) - b) - 1;
	}
	return cnt;
}
...
int main() {
	...
	lnt R = getValue(r);
	for (int i = 1; i <= n; i++) {
		int s = std::upper_bound(b + 1, b + n + 1, L - a[i]) - b;
		int e = std::lower_bound(b + 1, b + n + 1, R - a[i]) - b;
		for (int j = s; j < e; j++) c[++m] = a[i] + b[j];
	}
	std::sort(c + 1, c + m + 1);
	for (int i = count(L); i >= l; i--) printf("%lld ", L);
	for (int i = 1; i <= m; i++) printf("%lld ", c[i]);
	for (int i = count(R - 1) + 1; i <= r; i++) printf("%lld ", R);
	puts("");
	return 0;
}