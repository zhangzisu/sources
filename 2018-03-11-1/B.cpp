...
int n, m, q, type[MAXN], ddd[MAXN];
long long ans[MAXN], val[MAXN], w[MAXN];
std::string s;
...
int main() {
    ...
    dfs(1);
    for (int i = 1; i <= n; i++) {
        std::cin >> type[first[fuck[i]]];
        his[i].push_back(type[first[fuck[i]]]);
    }
    ...
    for (int i = 1; i <= rq; i++) std::cout << ans[i] << std::endl;
    return 0;
}